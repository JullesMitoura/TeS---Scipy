...
def GIBBS(data,Pmin,Pmax,Tmin,Tmax,eq,npressure,ntemp,inhibited_component=None):
    ...
    init = [(low + high)*0.8 / 2 for low, high in bnds]
    ...
    for i in range(len(P)):
        for j in range(len(T)):
            ...
            sol = minimize(gibbs, init, args=(T[j], P[i]), method='trust-constr', bounds=bnds, constraints=cons, options={'disp': False, 'maxiter': 100})
